...
def nuke_cut():
  ...
  if thisGroup is not nuke.root() and ( thisGroup.locked() or thisGroup.subgraphLocked() ) :
    ...
    raise RuntimeError("Can't cut because " + thisGroup.name() + " is " + lockedReason)
  nuke.nodeCopy(nukescripts.cut_paste_file())
  nukescripts.node_delete(popupOnError=True)

m.addCommand("Cut", "nuke_cut()", "^x", shortcutContext=dagContext)
...
m.addCommand("Paste Knob Values", "nukescripts.copy_knobs(\"\")", "^#V", shortcutContext=dagContext)
m.addCommand("Duplicate", "nuke.duplicateSelectedNodes()", "#c", shortcutContext=dagContext)
...
def nuke_declone():
  ...
  if thisGroup is not nuke.root() and ( thisGroup.locked() or thisGroup.subgraphLocked() ) :
    lockedReason = "published" if thisGroup.subgraphLocked() else "locked"
    raise RuntimeError("Can't declone because " + thisGroup.name() + " is " + lockedReason)

  selnodes = nuke.selectedNodes()
  for i in selnodes:
    nukescripts.declone(i)

m.addCommand("Declone", "nuke_declone()", "#+k", shortcutContext=dagContext)
m.addCommand("-", "", "")
...
# Add another one hidden to get Ctrl+Questionmark as well as Ctrl+Shift+Slash
n.addCommand("Filename/Search and Replace...", "nukescripts.search_replace()", "^+?", shortcutContext=dagContext)
n.addCommand("@;Search and Replace...", "nukescripts.search_replace()", "^+?" if isLinux else "^+/", shortcutContext=dagContext)
...
n.addCommand("Group/Collapse To LiveGroup", "nuke.collapseToLiveGroup()", "^l", shortcutContext=dagContext)
...
n.addCommand("Autoplace", "nuke.autoplace_all()", "l", shortcutContext=dagContext)
...
n.addCommand("Snap to Grid", "nuke.autoplace_snap_selected()", "Ctrl+\\", shortcutContext=dagContext)
n.addCommand("Snap All to Grid", "nuke.autoplace_snap_all()", "\\", shortcutContext=dagContext)
...
def _internal_expression_arrow_cmd():
  ...
  la = p.knob("link_knob_arrows");
  ea.setValue(not ea.value())
  ca.setValue(not ca.value())
  la.setValue(not la.value())
...
m.addCommand("Clear Blink Cache", nuke.clearBlinkCache)
...
m.addCommand("Nuke Plug-ins", "nukescripts.start('http://www.thefoundry.co.uk/products/nuke-product-family/nuke/nuke-plugins/')")
